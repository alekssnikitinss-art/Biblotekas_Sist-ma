...
function renderBooksAdmin(){ const container = document.getElementById('books-admin'); if(!container) return; const items = loadBooks(); container.innerHTML=''; items.forEach(b=>{ const div=document.createElement('div'); div.className='book'; const btnReturn = `<button onclick="adminReturnBook('${b.id}')" style="background:#10b981">📥 Atpakaļ pieejams</button>`; div.innerHTML = `<div>${b.image?'<img src="'+b.image+'">':'<div style="width:80px;height:100px;background:#eee;display:flex;align-items:center;justify-content:center">No image</div>'}</div><div style="flex:1"><strong>${escapeHtml(b.title)}</strong><div>${escapeHtml(b.author)}</div><div>ISBN: ${escapeHtml(b.isbn||'')}</div><div>Status: <strong>${escapeHtml(b.status)}</strong>${b.reserved_by?' ('+escapeHtml(b.reserved_by)+')':''}</div><div style="margin-top:6px"><button onclick="adminEdit('${b.id}')">✏️ Rediģēt</button> <button onclick="adminDelete('${b.id}')" style="background:#ef4444">🗑️ Dzēst</button> ${btnReturn} <button onclick="adminMarkFinished('${b.id}')" style="background:#6b7280">⚫ Pabeigts</button></div></div>`; container.appendChild(div); }); }
